...
docker exec --interactive "$container_id" \
  psql --dbname "$DATABASE_NAME" --set ON_ERROR_STOP=1 --username "$DATABASE_USER" \
  <"$REPOSITORY_ROOT/supabase/tests/ai-tools-artifacts.sql"

docker exec --interactive "$container_id" \
  psql --dbname "$DATABASE_NAME" --set ON_ERROR_STOP=1 --username "$DATABASE_USER" \
  <"$REPOSITORY_ROOT/supabase/tests/ai-execution-drafts.sql"

docker exec --interactive "$container_id" \
  psql --dbname "$DATABASE_NAME" --set ON_ERROR_STOP=1 --username "$DATABASE_USER" \
  <"$REPOSITORY_ROOT/supabase/tests/workflow-schedules.sql"

docker exec --interactive "$container_id" \
  psql --dbname "$DATABASE_NAME" --set ON_ERROR_STOP=1 --username "$DATABASE_USER" \
  <"$REPOSITORY_ROOT/supabase/tests/usage-store-operations.sql"

docker exec --interactive "$container_id" \
  psql --dbname "$DATABASE_NAME" --set ON_ERROR_STOP=1 --username "$DATABASE_USER" \
  <"$REPOSITORY_ROOT/supabase/tests/website-studio.sql"
