      ...
      <header className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo-600">
            <LocalizedText en="Workflow detail · Mock" zhHant="工作流詳情 · Mock" />
          </p>
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          <span className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 font-medium text-slate-700">
            ...
            v3 · {workflow.nodes.length} <LocalizedText en="nodes" zhHant="個節點" />
          </span>
          <span className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 font-medium text-slate-700">
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-950">
              <LocalizedText en="Visual workflow" zhHant="視覺化流程" />
            </h2>
            <p className="mt-1 text-xs text-slate-500">
              <LocalizedText
                en="Select a node to review its safety settings and approval rules."
                zhHant="選取節點以查看安全設定與核准規則。"
              />
            </p>
          </div>
          <span className="rounded-full bg-indigo-50 px-3 py-1 text-[11px] font-semibold text-indigo-700">
            <LocalizedText en="Current v3" zhHant="目前版本 v3" />
          </span>
        </div>
      ...
      <section className="mt-7 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-5 py-4 sm:px-6">
          <h2 className="text-base font-semibold text-slate-950">
            <LocalizedText en="Version history" zhHant="版本紀錄" />
          </h2>
          <p className="mt-1 text-xs text-slate-500">
            <LocalizedText
              en="Every change creates an immutable workflow version."
              zhHant="每次修改都建立不可變更的工作流版本。"
            />
          </p>
        </div>
        <div className="divide-y divide-slate-100">
          {MOCK_WORKFLOW_VERSIONS.map((version) => (
            ...
            >
              ...
              >
                {version.status === 'current' ? (
                  <LocalizedText en="Current" zhHant="目前版本" />
                ) : (
                  <LocalizedText en="Archived" zhHant="已封存" />
                )}
              </span>
            </Link>
          ))}
