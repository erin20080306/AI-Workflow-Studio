...
@media(max-width:800px){.site-header{align-items:flex-start;flex-direction:column;gap:10px;padding:16px 22px}.nav{max-width:100%;overflow:hidden}.nav-item{padding:6px 8px}.section{padding:56px 22px}.hero{min-height:auto}.hero-grid,.content-grid{grid-template-columns:1fr}.columns-3,.columns-4{grid-template-columns:repeat(2,minmax(0,1fr))}.footer-inner{align-items:flex-start;flex-direction:column}}@media(max-width:520px){.columns-2,.columns-3,.columns-4{grid-template-columns:1fr}.section-title{font-size:39px}.section-body{font-size:16px}.card{padding:22px}.asset{min-height:210px}.site-header{position:relative}}
`;
...
function assetMarkup(assetId: string | undefined, spec: WebsiteSpec): string {
...
function sectionMarkup(section: WebsiteSection, spec: WebsiteSpec): string {
  ...
  switch (section.type) {
    case 'hero':
      return `<section class="hero section ${section.layout}" id="${id}"><div class="section-inner ${
        ...
        section.layout === 'split' ? assetMarkup(section.assetId, spec) : ''
      }</div></section>`;
    case 'feature-grid':
    ...
    case 'stats':
      return `<section class="section stats" id="${id}"><div class="section-inner stat-grid">${section.items
        ...
        .join('')}</div></section>`;
    case 'testimonial':
      return `<section class="section" id="${id}"><div class="section-inner quote">“${escapeHtml(
        section.quote,
    ...
    case 'content':
      return `<section class="section" id="${id}"><div class="section-inner content-grid ${
        ...
        section.layout === 'text' ? '' : assetMarkup(section.assetId, spec)
...
export function renderWebsitePreviewDocument(specValue: WebsiteSpec, pageSlug: string): string {
  ...
  return `<!doctype html><html class="${escapeHtml(classes)}" lang="${escapeHtml(
    ...
    .map((section) => sectionMarkup(section, spec))
